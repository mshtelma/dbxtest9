...
```
dbx execute --cluster-name=<name of interactive cluster> --job=dbxtest9-sample-integration-test
```

For a test on a automated job cluster, use `launch` instead of `execute`:
```
dbx execute --job=dbxtest9-sample-integration-test
```
...
```
dbx launch --job=dbxtest9-sample
```
